...
// ─────────────────────────────────────────────────────────────────────────────
// Botón flotante del Coach que aparece en todas las secciones.
// Al abrirlo, el coach ya sabe dónde está el usuario y qué está viendo.
// No redirige a /coach — abre un mini-chat inline sin salir de la pantalla.
// ─────────────────────────────────────────────────────────────────────────────
...
// Mensajes de bienvenida contextuales según sección
function getWelcomeMessage(section, name) {
  ...
  const messages = {
    ...
    home:       `Buenos días${n}. Tengo tu resumen del día. ¿Por dónde empezamos?`,
...
export default function CoachFAB({ excludeRoutes = ['/coach'] }) {
  ...
  // Exponer recovery al api.js
  useEffect(() => {
    // El PandiStateContext ya expone recoveryScore
    // Lo publicamos en window para que api.js lo recoja
    try {
      const state = usePandiState()
      window.__pandi_recovery__ = state.recoveryScore
    } catch {}
  }, [recoveryLight])
  ...
  function handleOpen() {
    ...
    // Mensaje de bienvenida contextual si es la primera vez
  ...
  async function send(text) {
    ...
    try {
      const { reply } = await api.coach.chat(newMessages.slice(-8))
