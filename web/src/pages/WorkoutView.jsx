...
const PATHS = {
  ...
  warrior: {
    ...
    programs: [
      ...
      { id:'military', name:'Cardio HIIT Militar',     emoji:'🪖', tier:4, level:50,  duration:'40 min', desc:'Entrenamiento funcional de élite',                                     locked:false, mythic:false },
...
function GuidedSupportSession({ type, path, onClose, theme }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setTimeLeft(l => {
      if (l <= 1) { handleNext(); return 0 }
      return l - 1
    }), 1000)
    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <div className="h-1" style={{ background: theme.surface2 }}>
        <motion.div className="h-full" animate={{ width:`${progress}%` }}
          transition={{ duration:0.4 }} style={{ background: path.gradient }} />
      </div>
      ...
      {phase === 'active' && current && (
        <div className="flex-1 flex flex-col items-center justify-between px-4 py-6">
          ...
          {/* Ejercicio actual — FIX: sin y en animate */}
          <div className="flex flex-col items-center text-center">
            <ExerciseAnimation exerciseName={current.name} size={180} accent={path.accent} />
            <motion.h2
              key={idx + '_name'}
              initial={{ opacity:0 }}
              animate={{ opacity:1 }}
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] leading-tight" style={{ color: theme.textMuted }}>
          {isLocked && !program.mythic
            ...
            : program.mythic ? `🔐 Requiere nivel ${program.level}`
            : program.desc}
        </p>
        {!isLocked && (
...
export default function WorkoutView() {
  ...
  // FIX: UI inmediata — backend en background
    ...
    <div className="page pb-32" style={{ background: theme.bg }}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activePath}
          ...
          {/* Action Card */}
          <motion.button whileTap={{ scale:0.97 }} onClick={startFreeWorkout}
