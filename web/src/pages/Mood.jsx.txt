...
export default function Mood() {
  ...
  return (
    <div className="page">
      <h1 className="text-2xl font-extrabold mb-5">Estado emocional 💛</h1>

      <div className="card mb-5 space-y-5">
        <p className="font-semibold text-center">¿Cómo te sientes hoy?</p>
        <div className="flex justify-around">
          {MOODS.map(m => (
            <motion.button key={m.v} whileTap={{ scale: 0.85 }} onClick={() => { setMood(m.v); setSaved(false) }}
              className={`flex flex-col items-center gap-1 p-3 rounded-2xl transition-all ${
                mood === m.v ? 'bg-yellow-500/20 scale-110' : 'opacity-50'
              }`}>
        ...
        {[...Array(7)].map((_, i) => {
          ...
          return (
            <div key={i} className="flex-shrink-0 flex flex-col items-center gap-1 w-10">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl ${log ? 'bg-yellow-500/10' : 'bg-surface-3'}`}>
                {moodData?.emoji || '·'}
              </div>
              <span className="text-[10px] text-white/30">{d.getDate()}</span>
            </div>
          )
        })}
      </div>
