...
import { Camera, Upload, Check, RefreshCw, Search, Hash, X } from 'lucide-react'
        ...
        <div className="space-y-4">
          ...
          {!result && (
            <button onClick={analyze} disabled={analyzing} className="btn-primary flex items-center justify-center gap-2">
              {analyzing
                ? <><div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'rgba(255,255,255,0.3)', borderTopColor: '#fff' }} /> Analizando con IA…</>
            ...
            {result && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
                ...
                <div className="card space-y-2">
                  ...
                  <button onClick={() => { setPreview(null); setResult(null) }} className="btn-secondary text-sm py-2">Analizar otra foto</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
...
function BarcodeScanner({ onDetected, onClose }) {
  ...
  useEffect(() => {
    ...
    async function start() {
      if (!('BarcodeDetector' in window)) { setSupported(false); setStatus('Tu navegador no soporta el escáner. Usa Chrome en Android.'); return }
      try {
        ...
        async function scan() {
          if (!active || !videoRef.current || videoRef.current.readyState < 2) { rafRef.current = requestAnimationFrame(scan); return }
          try {
            const barcodes = await detector.detect(videoRef.current)
            if (barcodes.length > 0 && active) { active = false; stream.getTracks().forEach(t => t.stop()); onDetected(barcodes[0].rawValue); return }
          } catch {}
    ...
    start()
    return () => { active = false; if (rafRef.current) cancelAnimationFrame(rafRef.current); if (stream) stream.getTracks().forEach(t => t.stop()) }
  }, [])
...
export function EscanearTab({ onSaved }) {
  ...
  async function searchBarcode(code) {
    ...
    try { const data = await api.nutrition.barcode(q); setResult(data); setBarcode(q) }
    catch { setError('Producto no encontrado. Prueba con otro código.') }
  ...
  return (
    <>
      {scanning && <BarcodeScanner onDetected={(code) => { setScanning(false); searchBarcode(code) }} onClose={() => setScanning(false)} />}
      <div className="space-y-4">
        <div className="card" style={{ background: `${theme.primary}10`, border: `1px solid ${theme.primary}20` }}>
          <p className="font-semibold text-sm mb-1" style={{ color: theme.text }}>📦 Escáner de código de barras</p>
          <p className="text-xs" style={{ color: theme.textMuted }}>
            {nativeSupported ? 'Escanea con la cámara o introduce el código manualmente.' : 'Introduce el código manualmente.'}
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <input className="input flex-1" type="text" inputMode="numeric" placeholder="Ej: 8410188030032"
            value={barcode} onChange={e => setBarcode(e.target.value)} onKeyDown={e => e.key === 'Enter' && searchBarcode()} />
          <button onClick={() => searchBarcode()} disabled={loading || !barcode.trim()}
            className="w-12 h-12 rounded-xl flex items-center justify-center active:scale-90 transition-all disabled:opacity-40 flex-shrink-0"
            style={{ background: theme.primary }}>
            {loading ? <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'rgba(255,255,255,0.3)', borderTopColor: '#fff' }} />
              : <Search size={18} color="#fff" />}
          </button>
        </div>

        {error && <div className="card" style={{ background: `${theme.error}10`, border: `1px solid ${theme.error}20` }}><p className="text-sm" style={{ color: theme.error }}>{error}</p></div>}

        <AnimatePresence>
          {result && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
              <div className="card" style={{ background: `${theme.primary}10`, border: `1px solid ${theme.primary}20` }}>
                <p className="font-semibold mb-1" style={{ color: theme.text }}>{result.food_name}</p>
                <p className="text-xs mb-3" style={{ color: theme.textMuted }}>Por {result.serving_size || '100g'}</p>
                <div className="grid grid-cols-4 gap-2 text-center">
                  {[['🔥',result.calories,'kcal'],['💪',result.protein_g,'g prot'],['🌾',result.carbs_g,'g carbs'],['🫒',result.fat_g,'g grasa']].map(([e,v,u]) => (
                    <div key={u} className="rounded-xl py-2" style={{ background: theme.surface2 }}>
                      <p className="text-lg">{e}</p><p className="font-bold text-sm" style={{ color: theme.text }}>{Math.round(v)}</p>
                      <p className="text-[9px]" style={{ color: theme.textMuted }}>{u}</p>
                    </div>
                  ))}
                </div>
