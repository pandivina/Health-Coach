...
// Reemplaza RecetasTab en NutritionComponents.jsx
...
export function RecetasTab() {
  ...
  async function generate() {
    ...
    try {
      ...
      setRecipes(prev => [...(Array.isArray(data) ? data : [data]), ...prev])
      addXP(5)
    } catch (err) {
      if (err.message === 'premium_required') setShowPremium(true)
      else alert('Añade ingredientes en Despensa primero.')
  ...
  return (
    <div className="space-y-4">
      <motion.button whileTap={{ scale: 0.97 }} onClick={generate} disabled={generating}
        ...
        style={{ background: generating ? theme.surface2 : `linear-gradient(135deg, ${theme.primary}, #FF8FA3)`,
                 color: generating ? theme.textMuted : '#fff' }}>
        <Sparkles size={15} />
        {generating ? 'Generando recetas con IA…' : 'Generar recetas del día'}
      </motion.button>
      ...
      {generating && (
        <div className="card flex items-center gap-3">
          ...
          <p className="text-sm" style={{ color: theme.textMuted }}>
            Creando recetas con tus ingredientes…
          </p>
        </div>
      )}
      ...
      {recipes.length === 0 && !generating && (
        <div className="text-center py-10">
          ...
          <p className="font-semibold text-sm mb-1" style={{ color: theme.text }}>
            Tu caja Bento está vacía
          </p>
          <p className="text-xs" style={{ color: theme.textMuted }}>
            Añade ingredientes en Despensa y genera tus recetas del día
          </p>
        </div>
      )}

      <PremiumGate visible={showPremium}
        onClose={() => setShowPremium(false)}
        feature="Recetas con IA" />
    </div>
  )
}
