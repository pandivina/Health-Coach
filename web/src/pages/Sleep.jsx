...
const QUALITY_LABELS = {
  1: 'Terrible 😫', 2: 'Malo 😞', 3: 'Regular 😐', 4: 'Bueno 😊', 5: 'Excelente 🌟',
}

export default function Sleep() {
  const { user, addXP, healthProfile } = useStore()
  ...
  async function load() {
    ...
    if (todayLog) {
      setForm({ hours: String(todayLog.hours), quality: todayLog.quality, notes: todayLog.notes || '' })
      setSaved(true)
    }
  }
  ...
  // ── Coach ve: horas, calidad, media, horarios habituales ─────────────────
  useSectionContext('sleep', {
    lastNightHours:   lastNight?.hours   || null,
    lastNightQuality: lastNight?.quality || null,
    avgHours:         avgHours !== '–' ? parseFloat(avgHours) : null,
    todayRegistered:  !!todayLog,
    sleepTime:        healthProfile?.sleep_time  || null,
    wakeTime:         healthProfile?.wake_time   || null,
    sleepHoursTarget: healthProfile?.sleep_hours || 7,
  })

  return (
    <div className="page">
      <PandiContextualBubble section="sleep" data={{
        hours:   todayLog?.hours,
        quality: todayLog?.quality,
        avg:     avgHours !== '–' ? parseFloat(avgHours) : 7,
      }} />
      ...
      <div className="card mb-5 space-y-4">
        ...
        <input className="input" placeholder="¿Cómo te sientes?"
