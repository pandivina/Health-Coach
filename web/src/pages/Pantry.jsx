...
export default function Pantry() {
  ...
  async function scanReceipt(e) {
    ...
    try {
      ...
      reader.onload = async (ev) => {
        ...
        alert(`✅ Se añadieron ${result.inserted} ingredientes del ticket`)
        load()
      }
  ...
  return (
    <div className="page">
      <h1 className="text-2xl font-extrabold mb-5">Despensa 🛒</h1>

      {Object.entries(grouped).map(([cat, catItems]) => (
        <div key={cat} className="mb-5">
          <p className="text-white/50 text-xs uppercase tracking-wider font-semibold mb-2">{cat}</p>
          <div className="space-y-2">
            {catItems.map(item => (
              <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <button onClick={() => remove(item.id)} className="text-white/20 active:text-accent-red p-2">
                  <Trash2 size={14} />
                </button>
              </motion.div>
            ))}
      ...
      {items.length === 0 && <p className="text-white/30 text-center py-8">Tu despensa está vacía</p>}

      {showForm && (
        <div className="card mb-4 space-y-3">
          <input className="input" placeholder="Ingrediente" value={form.ingredient} onChange={e => setForm(f => ({ ...f, ingredient: e.target.value }))} />
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => fileRef.current?.click()} disabled={scanning} className="btn-secondary flex items-center justify-center gap-2">
          <Camera size={16} /> {scanning ? 'Leyendo…' : 'Ticket'}
        </button>
      </div>
    </div>
  )
