...
function PandiGreeting({ profile, theme, todayData }) {
  ...
  const messages = priorities.length > 0
    ? priorities.slice(0, 3)
    : greetings.map(t => ({ text: t, to: '/pet', color: theme.primary }))
      ...
      <div className="flex-1 relative">
        ...
        <AnimatePresence mode="wait">
          <motion.div key={msgIdx} initial={{ opacity: 0, x: 6 }} animate={{ opacity: 1, x: 0 }}
            ...
            <p className="text-[10px] font-bold mb-0.5" style={{ color: current.color }}>{petName}</p>
            <p className="text-xs leading-relaxed" style={{ color: theme.text }}>{current.text}</p>
            {messages.length > 1 && (
...
function NotificationCard({ theme }) {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('pandi_notif_dismissed')) setDismissed(true)
  }, [])
...
export default function Home() {
  ...
  // ── Coach ve el resumen completo del día en tiempo real ──────────────────
  useSectionContext('home', {
    caloriesConsumed: Math.round(cals),
    caloriesTarget:   goals.calories,
    proteinConsumed:  Math.round(protein),
    proteinTarget:    goals.protein_g,
    caloriesBurned:   burned,
    waterGlasses,
    waterGoal:        8,
    workedOutToday:   !!todayWorkout,
    sleepLastNight:   todaySleep?.hours   || null,
    sleepQuality:     todaySleep?.quality || null,
    moodToday:        todayMood?.mood     || null,
    streak:           profile?.streak     || 0,
    level:            profile?.level      || 1,
    modulesCompleted: [
      !!todayMood, cals > 0, !!todaySleep, !!todayWorkout, waterGlasses > 0,
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            <div className="flex flex-col items-center px-2.5 py-1.5 rounded-2xl" style={{ background: theme.surface2 }}>
              ...
              <span className="font-extrabold text-sm" style={{ color: theme.primary }}>{profile?.level || 1}</span>
            </div>
            <div className="flex flex-col items-center px-2.5 py-1.5 rounded-2xl" style={{ background: theme.surface2 }}>
