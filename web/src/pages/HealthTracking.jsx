...
function WeightTab() {
  ...
  const [logs, setLogs] = useState([]); const [health, setHealth] = useState(null)
  const [form, setForm] = useState({ weight_kg: '', body_fat_pct: '', notes: '' }); const [saving, setSaving] = useState(false)
  async function load() {
    ...
    setLogs(lr.data || []); if (hr.data) setHealth(hr.data)
  }
  useEffect(() => { if (user) load() }, [user])
  async function save() {
    if (!form.weight_kg) return; setSaving(true)
    try { await api.health.logWeight({ weight_kg: parseFloat(form.weight_kg), body_fat_pct: parseFloat(form.body_fat_pct) || null, notes: form.notes }); setForm({ weight_kg: '', body_fat_pct: '', notes: '' }); load() }
    catch (err) { alert(err.message) } finally { setSaving(false) }
  }
  const current = logs[0]?.weight_kg; const previous = logs[1]?.weight_kg
      ...
      {health && (
        <div className="card" style={{ background: `${theme.primary}10`, border: `1px solid ${theme.primary}20` }}>
          <div className="grid grid-cols-3 gap-3 text-center">
            {[[current || health.weight_kg || '–','kg actuales'],[health.target_weight_kg || '–','kg objetivo'],[toGoal || '–','kg restantes']].map(([v,l]) => (
              <div key={l}><p className="text-2xl font-bold" style={{ color: theme.text }}>{v}</p><p className="text-xs" style={{ color: theme.textMuted }}>{l}</p></div>
            ))}
          </div>
          {diff && (
            <div className="mt-3 pt-3 flex items-center justify-center gap-2" style={{ borderTop: `1px solid ${theme.border}` }}>
              {diff < 0 ? <TrendingDown size={14} style={{ color: theme.success }} /> : diff > 0 ? <TrendingUp size={14} style={{ color: theme.warning }} /> : <Minus size={14} style={{ color: theme.textMuted }} />}
              <span className="text-sm font-medium" style={{ color: diff < 0 ? theme.success : theme.warning }}>{diff > 0 ? '+' : ''}{diff} kg desde el último</span>
            </div>
          )}
        </div>
      )}
      <div className="card space-y-3">
        <p className="font-semibold text-sm" style={{ color: theme.text }}>Registrar peso</p>
        <div className="flex gap-2">
          <div className="flex-1"><label className="label">Peso (kg)</label><input className="input" type="number" step="0.1" placeholder="70.5" value={form.weight_kg} onChange={e => setForm(f => ({ ...f, weight_kg: e.target.value }))} /></div>
          <div className="flex-1"><label className="label">% grasa</label><input className="input" type="number" step="0.1" placeholder="20" value={form.body_fat_pct} onChange={e => setForm(f => ({ ...f, body_fat_pct: e.target.value }))} /></div>
        </div>
        <input className="input text-sm" placeholder="Notas (opcional)" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
        <button onClick={save} disabled={saving || !form.weight_kg} className="btn-primary text-sm py-2.5 disabled:opacity-40">⚖️ Registrar peso</button>
      </div>
      <p className="section-title">Historial</p>
      {logs.map((log, i) => {
        const prev = logs[i+1]?.weight_kg; const d = prev ? (log.weight_kg - prev).toFixed(1) : null
        return (
          <div key={log.id} className="card flex items-center justify-between">
            <div><p className="font-semibold" style={{ color: theme.text }}>{log.weight_kg} kg</p><p className="text-xs" style={{ color: theme.textMuted }}>{new Date(log.date + 'T12:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}</p></div>
            <div className="text-right">{log.body_fat_pct && <p className="text-xs" style={{ color: theme.textMuted }}>{log.body_fat_pct}% grasa</p>}{d && <p className="text-xs font-medium" style={{ color: d < 0 ? theme.success : theme.warning }}>{d > 0 ? '+' : ''}{d}</p>}</div>
          </div>
        )
      })}
    </div>
...
function MeasuresTab() {
  ...
  useEffect(() => { if (user) supabase.from('body_measurements').select('*').eq('user_id', user.id).order('date', { ascending: false }).limit(6).then(({ data }) => setLogs(data || [])) }, [user])
  async function save() {
    setSaving(true)
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="card space-y-3">
        ...
        <input className="input text-sm" placeholder="Notas…" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
        <button onClick={save} disabled={saving} className="btn-primary text-sm py-2.5">📏 Guardar medidas</button>
      </div>
      {logs.map(m => (
        <div key={m.id} className="card">
          <p className="text-xs mb-2" style={{ color: theme.textMuted }}>{new Date(m.date + 'T12:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'long' })}</p>
...
function LabsTab() {
  ...
  async function load() { const { data } = await supabase.from('lab_reports').select('*, lab_markers(*)').eq('user_id', user.id).order('report_date', { ascending: false }); setReports(data || []) }
  useEffect(() => { if (user) load() }, [user])
  async function analyze(imageBase64, mediaType) {
    setAnalyzing(true)
    try { await api.labs.analyze({ rawText, imageBase64, mediaType, title: reportTitle || 'Analítica' }); setRawText(''); setReportTitle(''); load() }
    catch (err) { alert('Error: ' + err.message) } finally { setAnalyzing(false) }
  }
      ...
      <div className="card space-y-3">
        ...
        <div className="flex gap-2">
          {[['text','✍️ Texto'],['image','📷 Imagen']].map(([v,l]) => (
            <button key={v} onClick={() => setMode(v)} className="flex-1 py-2 rounded-xl border text-sm font-medium transition-all"
              style={{ borderColor: mode===v ? theme.primary : theme.border, background: mode===v ? `${theme.primary}15` : theme.surface2, color: mode===v ? theme.primary : theme.textMuted }}>{l}</button>
          ))}
        </div>
        <input className="input text-sm" placeholder="Título (ej: Analítica enero)" value={reportTitle} onChange={e => setReportTitle(e.target.value)} />
        {mode === 'text' ? (
          <>
            <textarea className="input resize-none text-sm" rows={6} placeholder="Pega aquí el texto de tu analítica…" value={rawText} onChange={e => setRawText(e.target.value)} />
            <button onClick={() => analyze(null, null)} disabled={analyzing || !rawText.trim()} className="btn-primary text-sm py-2.5 disabled:opacity-40 flex items-center justify-center gap-2">
              {analyzing ? <><div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'rgba(255,255,255,0.3)', borderTopColor: '#fff' }} /> Analizando…</> : '🔬 Analizar con IA'}
            </button>
          </>
        ) : (
          <>
            <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleFile} />
            <button onClick={() => fileRef.current?.click()} disabled={analyzing} className="btn-primary text-sm py-2.5 flex items-center justify-center gap-2 disabled:opacity-40">
              {analyzing ? <><div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'rgba(255,255,255,0.3)', borderTopColor: '#fff' }} /> Analizando…</> : <><Upload size={14} /> Subir imagen</>}
            </button>
          </>
        )}
      </div>
      {reports.map(report => (
        <div key={report.id} className="card cursor-pointer" onClick={() => setExpanded(expanded === report.id ? null : report.id)}>
          <div className="flex items-start justify-between">
            <div>
              <p className="font-semibold" style={{ color: theme.text }}>{report.title}</p>
              <p className="text-xs" style={{ color: theme.textMuted }}>{new Date(report.report_date + 'T12:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: report.status==='analyzed' ? `${theme.success}20` : `${theme.warning}20`, color: report.status==='analyzed' ? theme.success : theme.warning }}>
                {report.status === 'analyzed' ? '✓ Analizada' : 'Pendiente'}
              </span>
              <button onClick={e => { e.stopPropagation(); api.labs.deleteReport(report.id).then(load) }} style={{ color: theme.textLight }}><Trash2 size={13} /></button>
            </div>
          </div>
          {expanded === report.id && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 space-y-3">
              {report.ai_interpretation && <p className="text-sm leading-relaxed" style={{ color: theme.text }}>{report.ai_interpretation}</p>}
...
function TreatmentsTab() {
  const { user } = useStore(); const { theme } = useTheme()
  const [treatments, setTreatments] = useState([]); const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({ name:'', type:'other', dose:'', frequency:'', notes:'', start_date:'' })
  async function load() { const { data } = await supabase.from('medical_treatments').select('*').eq('user_id', user.id).eq('active', true).order('created_at', { ascending: false }); setTreatments(data || []) }
  useEffect(() => { if (user) load() }, [user])
  async function add() {
    if (!form.name) return
    ...
    setForm({ name:'', type:'other', dose:'', frequency:'', notes:'', start_date:'' }); setShowForm(false); load()
  }
  ...
  return (
    <div className="space-y-4">
      <button onClick={() => setShowForm(true)} className="btn-primary flex items-center justify-center gap-2"><Plus size={15} /> Añadir tratamiento</button>
      {showForm && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card space-y-3">
          <input className="input" placeholder="Nombre del tratamiento" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} autoFocus />
          ...
          <div className="flex gap-2">
            ...
            <input className="input flex-1" placeholder="Frecuencia" value={form.frequency} onChange={e => setForm(f => ({ ...f, frequency: e.target.value }))} />
          </div>
      ...
      {treatments.map(t => (
        <div key={t.id} className="card flex items-start justify-between">
          ...
          <button onClick={() => supabase.from('medical_treatments').update({ active: false }).eq('id', t.id).then(load)} style={{ color: theme.textLight }}><Trash2 size={13} /></button>
        </div>
      ))}
      {treatments.length === 0 && (
        <div className="text-center py-8" style={{ color: theme.textMuted }}>
