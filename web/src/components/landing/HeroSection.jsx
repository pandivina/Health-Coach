...
// Posiciones de partículas precalculadas (sin Math.random en render)
  ...
  return (
    <div style={{ position: 'relative', width: 220, height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {[1, 2, 3].map(ring => (
        <motion.div key={ring}
          style={{
            position: 'absolute', width: 140 + ring * 40, height: 140 + ring * 40,
      ...
      <motion.div
        style={{
          position: 'relative', zIndex: 2, width: 130, height: 130, borderRadius: '50%',
      ...
      >
        {imgErr
          ...
          : <img src="/panda/avatar_happy.png" alt="Pandi"
  ...
  return (
    <>
      <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut' }}
        style={{ position: 'absolute', right: -20, top: 40, background: 'white', borderRadius: 16, padding: '10px 14px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', border: `1px solid ${C.border}`, minWidth: 100 }}>
        ...
        <div style={{ display: 'flex', gap: 3, marginTop: 4 }}>
          {[...Array(7)].map((_, i) => (
            <div key={i} style={{ width: 8, height: 8, borderRadius: 2, background: i < 6 ? C.primary : C.border }} />
          ))}
        </div>
      </motion.div>
      <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
        style={{ position: 'absolute', left: -24, bottom: 60, background: 'white', borderRadius: 16, padding: '10px 14px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', border: `1px solid ${C.border}`, minWidth: 100 }}>
      ...
      <motion.div animate={{ y: [0, -6, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
        style={{ position: 'absolute', left: -16, top: 20, background: 'white', borderRadius: 14, padding: '8px 12px', boxShadow: '0 6px 24px rgba(0,0,0,0.1)', border: `1px solid ${C.border}` }}>
...
function DemoChat() {
  const messages = [
    ...
    { user: false, text: 'Con Ozempic tu apetito es menor: 150g pollo + arroz. Energía sin saturarte. ¿A qué hora entrenas?' },
    { user: true,  text: 'En 30 minutos' },
    { user: false, text: 'Come ahora. Tu proteína hoy va al 68%. Después: batido o requesón.' },
  ]
  return (
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6, duration: 0.6 }}
      style={{ background: 'white', borderRadius: 24, border: `1px solid ${C.border}`, boxShadow: '0 8px 48px rgba(0,0,0,0.08)', overflow: 'hidden', maxWidth: 320, width: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderBottom: `1px solid ${C.border}`, background: 'linear-gradient(135deg, #f0fffe, #fff5f7)' }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: C.grad, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>🐼</div>
        <div>
      ...
      <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {messages.map((msg, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 + i * 0.3 }}
            style={{ display: 'flex', justifyContent: msg.user ? 'flex-end' : 'flex-start' }}>
            <div style={{
              maxWidth: '82%', padding: '8px 12px', borderRadius: 14, fontSize: 12, lineHeight: 1.45,
              background: msg.user ? C.primary : '#F5F7FA', color: msg.user ? '#fff' : C.text,
              borderBottomRightRadius: msg.user ? 3 : 14, borderBottomLeftRadius: msg.user ? 14 : 3,
            }}>{msg.text}</div>
          </motion.div>
        ))}
...
export default function HeroSection() {
  ...
  useEffect(() => {
    const t = setInterval(() => setStateIdx(i => (i + 1) % PANDI_STATES.length), 4000)
  ...
  return (
    <section style={{ position: 'relative', overflow: 'hidden', minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
      ...
      <motion.div animate={{ x: [0, 30, 0], y: [0, -20, 0] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
        style={{ position: 'absolute', top: '-10%', right: '-5%', width: 400, height: 400, borderRadius: '50%', background: `radial-gradient(circle, ${state.aura[0]}30 0%, transparent 70%)`, filter: 'blur(40px)', zIndex: 0 }} />
      <motion.div animate={{ x: [0, -20, 0], y: [0, 30, 0] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
        style={{ position: 'absolute', bottom: '-10%', left: '-5%', width: 350, height: 350, borderRadius: '50%', background: `radial-gradient(circle, ${C.pink}25 0%, transparent 70%)`, filter: 'blur(40px)', zIndex: 0 }} />

      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '6rem 1.5rem 5rem', position: 'relative', zIndex: 1, width: '100%' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 48 }}>

          <div style={{ flex: 1, textAlign: 'center', maxWidth: 600 }}>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 16px', borderRadius: 20, background: `${C.primary}15`, border: `1px solid ${C.primary}30`, fontSize: 12, fontWeight: 700, color: C.primary, marginBottom: 20 }}>
              <Zap size={12} />
            ...
            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }}
              style={{ fontSize: 'clamp(2.4rem, 6vw, 4rem)', fontWeight: 900, lineHeight: 1.1, color: C.text, margin: '0 0 20px', letterSpacing: '-0.02em' }}>
              Pandi no registra<br />tu salud.{' '}
            ...
            <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
              style={{ fontSize: 'clamp(1rem, 2vw, 1.2rem)', color: C.muted, lineHeight: 1.7, margin: '0 0 16px' }}>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
                <Link to="/auth" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 18, background: C.grad, color: '#fff', fontWeight: 800, fontSize: 15, textDecoration: 'none', boxShadow: `0 8px 32px ${C.primary}50` }}>
                  Dale vida a tu Pandi <ArrowRight size={16} />
                </Link>
                <a href="#como-funciona" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 24px', borderRadius: 18, background: 'rgba(255,255,255,0.85)', border: `1.5px solid ${C.border}`, color: C.text, fontWeight: 600, fontSize: 14, textDecoration: 'none' }}>
                  Ver cómo funciona
                </a>
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.7 }}
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 32, width: '100%', maxWidth: 480 }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
              {PANDI_STATES.map((s, i) => (
                <button key={s.id} onClick={() => setStateIdx(i)}
                  style={{ padding: '6px 14px', borderRadius: 20, cursor: 'pointer', border: `2px solid ${stateIdx === i ? s.aura[0] : C.border}`, background: stateIdx === i ? `${s.aura[0]}15` : 'rgba(255,255,255,0.7)', color: stateIdx === i ? s.aura[0] : C.muted, fontSize: 11, fontWeight: 700, transition: 'all 0.2s' }}>
