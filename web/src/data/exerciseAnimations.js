...
export const EXERCISE_ANIMATIONS = {
  ...
  'w_sentadilla_salto': {
    name: 'Sentadilla con Salto', fps: 600,
    frames: [
      `<g stroke="#2EC4B6" stroke-width="4" stroke-linecap="round" fill="none">
  ...
  // ── ZEN ───────────────────────────────────────────────────────────────────
  'm_respiracion': {
    name: 'Respiración 4-7-8', fps: 1500,
    frames: [
  ...
  'z_perro_boca_abajo': {
    name: 'Perro Boca Abajo', fps: 1200,
    frames: [
      `<g stroke="#6EE7B7" stroke-width="4" stroke-linecap="round" fill="none">
        <circle cx="50" cy="55" r="10"/>
        <line x1="50" y1="65" x2="50" y2="110"/>
  ...
  'warmup_rotacion_hombros': {
    name: 'Rotación de hombros', fps: 500,
    frames: [
      `<g stroke="#2EC4B6" stroke-width="4" stroke-linecap="round" fill="none">
        <circle cx="50" cy="30" r="10"/>
        <line x1="50" y1="40" x2="50" y2="95"/>
  ...
  // ── CALENTAMIENTO / ESTIRAMIENTOS GUERRERO (nuevas) ───────────────────────
...
export function getAnimationByName(name) {
  ...
  // 1. Match exacto
  const exact = Object.values(EXERCISE_ANIMATIONS).find(
    a => a.name.toLowerCase() === q
  )
  if (exact) return exact

  // 2. Match parcial
  const partial = Object.values(EXERCISE_ANIMATIONS).find(
    a => a.name.toLowerCase().includes(q) || q.includes(a.name.toLowerCase())
  )
  if (partial) return partial

  // 3. Match por palabra clave principal (>4 caracteres)
  const mainWord = q.split(' ').find(w => w.length > 4)
  if (mainWord) {
    const keyword = Object.values(EXERCISE_ANIMATIONS).find(
      a => a.name.toLowerCase().includes(mainWord)
    )
