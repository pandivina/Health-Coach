        ...
        <div className="p-3 space-y-2.5 overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-lg"
              style={{ background: C.surface2 }}>🐼</div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {[['🍎','Nutrición',C.primary],['💪','Entrena','#F97316'],['😴','Sueño','#6366f1'],['💧','Agua',C.primary]].map(([e,l,col]) => (
              <div key={l} className="rounded-xl p-1.5 flex flex-col items-center gap-0.5"
          ...
          <div className="rounded-xl p-2.5" style={{ background: `${C.primary}15`, border: `1px solid ${C.primary}30` }}>
            ...
            <p style={{ color: C.text, fontSize: 8, lineHeight: 1.4 }}>
              "María, llevas 3 días sin alcanzar tu proteína objetivo. Con tu tratamiento con Levotiroxina, es especialmente importante…"
            </p>
          </div>
...
const FEATURES = [
  {
    icon: Brain, color: C.primary,
    title: 'Coach IA contextual',
    desc: 'El único coach que conoce tus analíticas, tratamientos, historial de peso y objetivos. Respuestas reales, no genéricas.',
    badge: 'Diferenciador clave',
  },
  {
    icon: Apple, color: '#F97316',
    title: 'Nutrición inteligente',
    desc: 'Analiza fotos de tus platos, escanea códigos de barras y genera recetas con tus ingredientes disponibles.',
    badge: null,
  },
  {
    icon: Dumbbell, color: '#6366f1',
    title: 'Entrenamiento con PRs',
    desc: 'Rutinas generadas por IA, seguimiento de récords personales y sesiones en tiempo real con timer de descanso.',
    badge: null,
  },
  {
    icon: FlaskConical, color: C.pink,
    title: 'Analíticas clínicas',
    desc: 'Sube tus análisis de sangre y la IA extrae marcadores clave (hierro, vitamina D, glucosa) con recomendaciones nutricionales.',
    badge: '✨ Premium',
  },
  {
    icon: Moon, color: '#818cf8',
    title: 'Sueño y bienestar',
    desc: 'Seguimiento de sueño, ánimo e hidratación. La IA cruza todos los datos para darte el cuadro completo de tu salud.',
    badge: null,
  },
  {
    icon: Trophy, color: '#F59E0B',
    title: 'Gamificación real',
    desc: '21 logros conectados a acciones reales, XP, niveles y mascotas. Mantén la motivación sin trucos baratos.',
    badge: null,
  },
]
...
const STEPS = [
  ...
  { n: '02', title: 'La IA se personaliza', desc: 'Calculamos tu TDEE, tus macros óptimos y configuramos el Coach con toda la información clínica que has proporcionado.' },
...
const FAQS = [
  ...
  { q: '¿El Coach IA sustituye a un médico o nutricionista?', a: 'No. Health Coach es una herramienta de apoyo y seguimiento. Las recomendaciones son orientativas y no sustituyen el consejo de un profesional de la salud. Siempre consulta a tu médico antes de realizar cambios significativos.' },
  { q: '¿Puedo usar la app si tengo tratamientos médicos?', a: 'Sí, y de hecho es donde más brilla. Registra tus tratamientos (GLP-1, tiroides, anticonceptivos…) y el Coach los tendrá en cuenta al calcular tus objetivos y recomendaciones.' },
      ...
      <nav className="sticky top-0 z-50" style={{ background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${C.border}` }}>
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            {['Funcionalidades','Cómo funciona','Precio'].map(l => (
              <a key={l} href={`#${l.toLowerCase().replace(' ','-')}`}
                className="text-sm font-medium transition-colors"
                style={{ color: C.muted }}>{l}</a>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <Link to="/auth" className="text-sm font-medium hidden md:block" style={{ color: C.muted }}>
              Iniciar sesión
            </Link>
            <Link to="/auth"
            ...
            <div className="flex-1 text-center lg:text-left">
              ...
              <FadeIn delay={0.1}>
                <h1 className="font-extrabold leading-tight mb-4"
                  style={{ fontSize: 'clamp(2.2rem, 5vw, 3.5rem)', color: C.text }}>
                  El coach que sí<br />
                  <GradText>te conoce de verdad</GradText>
                </h1>
              </FadeIn>
              ...
              <FadeIn delay={0.3}>
                <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start mb-6">
                  <Link to="/auth"
                    className="flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl text-white font-bold text-base transition-all active:scale-95"
                    style={{ background: C.grad, boxShadow: `0 8px 24px ${C.primary}40` }}>
                    Empieza gratis <ArrowRight size={16} />
                  </Link>
                  <a href="#como-funciona"
                    className="flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl font-semibold text-base transition-all"
              ...
              <FadeIn delay={0.4}>
                <p className="text-xs" style={{ color: C.light }}>
                  ✓ 7 días Premium gratis &nbsp;·&nbsp; ✓ Sin tarjeta de crédito &nbsp;·&nbsp; ✓ Cancela cuando quieras
                </p>
              </FadeIn>
            </div>
      ...
      <section style={{ background: C.surface, padding: '2.5rem 0', borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
        <div className="max-w-5xl mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-8 items-center">
            {[
              { v: '+500', l: 'usuarios en beta' },
              { v: '4.9★', l: 'valoración media' },
              { v: '−4.8kg', l: 'pérdida media en 90 días' },
              { v: '94%', l: 'mejoran sus hábitos en 30 días' },
            ].map(({ v, l }) => (
      ...
      <section style={{ padding: '5rem 1rem' }}>
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-base mb-12" style={{ color: C.muted }}>
            Todas calculan calorías. Ninguna sabe que tomas Levotiroxina, que trabajas de noche o que tu ferritina está baja.
          </p>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { emoji: '🤖', title: 'Consejos de copia-pega', desc: '"Come 2.000 kcal y haz ejercicio." El mismo plan para todos. Sin importar tus circunstancias.' },
              { emoji: '📊', title: 'Datos sin contexto', desc: 'Te muestran gráficas bonitas pero no te dicen qué hacer con ellas ni por qué están pasando.' },
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="font-extrabold mb-4" style={{ fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', color: C.text }}>
              ...
              <GradText>conectado e inteligente</GradText>
            </h2>
            <p className="text-base max-w-xl mx-auto" style={{ color: C.muted }}>
              Seis módulos que trabajan juntos y alimentan a un Coach IA que sí te conoce.
            </p>
          </div>
      ...
      <section id="como-funciona" style={{ padding: '5rem 1rem' }}>
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="font-extrabold" style={{ fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', color: C.text }}>
              En marcha en menos de 2 minutos
            </h2>
          </div>
          <div className="space-y-6">
            {STEPS.map(({ n, title, desc }, i) => (
              <motion.div key={n} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }}
                ...
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-extrabold text-sm flex-shrink-0"
          ...
          <div className="rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center gap-10"
            ...
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-semibold mb-5"
                ...
                <Brain size={13} /> Coach IA contextual
              </div>
              <h2 className="font-extrabold mb-4" style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)', color: C.text }}>
                El coach que sabe lo que<br />ninguna app te pregunta
              </h2>
              <p className="mb-6" style={{ color: C.muted, lineHeight: 1.7 }}>
                Pregúntale qué comer antes de entrenar teniendo en cuenta que tomas Ozempic y que tienes la vitamina D baja.
                Él lo sabe. Y te responde con eso en mente.
              </p>
              <div className="space-y-3">
                {[
                  'Conoce tus analíticas y marcadores clínicos',
                  'Tiene en cuenta tus tratamientos médicos activos',
                  'Adapta los consejos a tu horario laboral y sueño',
                  'Cruza nutrición, entrenamiento y bienestar',
                ].map(f => (
            ...
            <div className="w-full md:w-72 space-y-3">
              {[
                ...
                { user: false, text: '¡Claro! Dado que llevas Ozempic y tu apetito puede ser menor, te recomiendo algo ligero pero rico en proteína: salmón al horno con espinacas. Unos 180g de salmón te darán ≈36g proteína y es fácil de digerir a la hora de cenar. Evita carbohidratos pesados esta noche.' },
              ].map(({ user, text }, i) => (
                <div key={i} className={`flex ${user ? 'justify-end' : 'justify-start'}`}>
                  {!user && (
                    <div className="w-7 h-7 rounded-xl flex items-center justify-center text-sm mr-2 flex-shrink-0 mt-1"
                      style={{ background: `${C.primary}20` }}>🤖</div>
                  )}
                  <div className="rounded-2xl px-4 py-3 text-sm max-w-[85%]" style={{
                    background: user ? C.primary : C.surface,
                    color: user ? '#fff' : C.text,
      ...
      <section style={{ padding: '5rem 1rem' }}>
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {[
              { name: 'Laura M.', role: 'Enfermera, turno nocturno', avatar: '👩‍⚕️', stars: 5, text: 'Por fin una app que entiende que trabajo de noche y que mis horarios de comida son distintos. El Coach me adapta las recomendaciones a mis turnos. Llevo 8 semanas y he perdido 3.2kg sin pasar hambre.' },
              { name: 'Carlos R.', role: 'Lleva Ozempic desde enero', avatar: '👨', stars: 5, text: 'Con el GLP-1 mi apetito cayó mucho y no sabía cómo comer bien con tan pocas calorías. El Coach tiene en cuenta el tratamiento y me ayuda a priorizar proteína. He ganado masa muscular mientras perdía grasa.' },
              { name: 'Ana T.', role: 'Tiroides, analíticas irregulares', avatar: '👩', stars: 5, text: 'Subí mis analíticas y la IA detectó que tenía ferritina baja y vitamina D escasa. Me dio recomendaciones nutricionales específicas. Mi médica se sorprendió de lo bien que entendí mis propios resultados.' },
              ...
              <motion.div key={name} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}
                ...
                <div className="flex gap-0.5 mb-4">
                  {[...Array(stars)].map((_, i) => (
                    <Star key={i} size={14} fill={C.primary} style={{ color: C.primary }} />
                  ))}
                </div>
                <p className="text-sm mb-5 leading-relaxed" style={{ color: C.text }}>"{text}"</p>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center text-xl"
                    style={{ background: C.surface2 }}>{avatar}</div>
                  <div>
      ...
      <section id="precio" style={{ background: C.surface, padding: '5rem 1rem' }}>
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="font-extrabold mb-3" style={{ fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', color: C.text }}>
              Empieza gratis.<br />
              <GradText>Evoluciona cuando quieras.</GradText>
            </h2>
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2.5 mb-6">
                {['Diario nutricional manual','Coach IA (10 mensajes/día)','Entrenamiento básico','Sueño, ánimo e hidratación','Mascota Panda','Informe diario básico'].map(f => (
                  <div key={f} className="flex items-center gap-2">
                    <Check size={14} style={{ color: C.primary, flexShrink: 0 }} />
                    <p className="text-sm" style={{ color: C.text }}>{f}</p>
                  </div>
                ))}
              </div>
              <Link to="/auth" className="block w-full text-center py-3 rounded-xl font-semibold text-sm transition-all"
            ...
            <div className="rounded-2xl p-7 relative" style={{ background: C.text, border: `2px solid ${C.primary}` }}>
              ...
              <div className="space-y-2.5 mb-6">
                {['Todo lo del plan gratuito','Coach IA ilimitado + contexto clínico','Análisis de foto de comida','Escáner de código de barras','Generación de recetas personalizadas','Interpretación de analíticas con IA','Todas las mascotas desbloqueadas'].map(f => (
                  <div key={f} className="flex items-center gap-2">
              ...
              <Link to="/auth" className="block w-full text-center py-3 rounded-xl font-bold text-sm text-white transition-all active:scale-97"
                style={{ background: C.grad }}>
                Probar 7 días gratis
              </Link>
            </div>
          </div>
          <p className="text-center text-xs mt-4" style={{ color: C.light }}>
            El período de prueba no requiere tarjeta de crédito
          </p>
        </div>
      </section>
      ...
      <section style={{ background: C.surface, padding: '5rem 1rem' }}>
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className="font-extrabold" style={{ fontSize: 'clamp(1.6rem, 3vw, 2rem)', color: C.text }}>
      ...
      <section style={{ padding: '5rem 1rem' }}>
        <div className="max-w-2xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            style={{ background: C.gradSoft, border: `1px solid ${C.primary}30` }}>
            <div className="text-5xl mb-4">🐼</div>
            <h2 className="font-extrabold mb-4" style={{ fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', color: C.text }}>
              Tu salud merece<br />
              <GradText>un coach que te conoce</GradText>
            </h2>
            <p className="mb-8" style={{ color: C.muted, lineHeight: 1.7 }}>
              Únete a más de 500 personas que ya están mejorando su salud con datos reales.<br />
              Empieza hoy. Gratis. Sin excusas.
            </p>
            <Link to="/auth"
              className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl text-white font-bold text-base transition-all active:scale-95"
              style={{ background: C.grad, boxShadow: `0 12px 32px ${C.primary}40` }}>
              Empezar ahora — es gratis <ArrowRight size={16} />
            </Link>
            <p className="text-xs mt-4" style={{ color: C.light }}>
              ✓ 7 días Premium incluidos · ✓ Sin tarjeta · ✓ Cancela cuando quieras
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-8">
            <div>
              ...
              <p className="text-sm max-w-xs" style={{ color: 'rgba(255,255,255,0.5)', lineHeight: 1.6 }}>
                Coaching de salud con IA personalizado con tus datos reales.
              </p>
              <p className="text-xs mt-3 max-w-xs" style={{ color: 'rgba(255,255,255,0.3)', lineHeight: 1.5 }}>
                ⚕️ Health Coach no proporciona consejo médico. La información generada es orientativa y no sustituye la opinión de un profesional sanitario.
              </p>
            </div>
            <div className="flex gap-12">
              <div>
                <p className="text-xs font-bold mb-3 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Producto</p>
                {['Funcionalidades','Precio','Privacidad'].map(l => (
                  <p key={l} className="text-sm mb-2 cursor-pointer" style={{ color: 'rgba(255,255,255,0.6)' }}>{l}</p>
                ))}
              </div>
              <div>
                <p className="text-xs font-bold mb-3 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Legal</p>
                {['Términos de uso','Política de privacidad','Cookies','Disclaimer médico'].map(l => (
                  <p key={l} className="text-sm mb-2 cursor-pointer" style={{ color: 'rgba(255,255,255,0.6)' }}>{l}</p>
                ))}
              </div>
            </div>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '1.5rem' }}
            ...
            <p className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>
              © 2026 Health Coach. Todos los derechos reservados.
            </p>
            <p className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>
